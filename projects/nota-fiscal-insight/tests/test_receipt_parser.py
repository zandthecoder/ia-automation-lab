...
NON_CONVERTIBLE_UNIT_PRICE_FIXTURE_PATH = (
    PROJECT_ROOT / "fixtures" / "inputs" / "non_convertible_unit_price.txt"
)
NEGATIVE_LINE_TOTAL_FIXTURE_PATH = (
    PROJECT_ROOT / "fixtures" / "inputs" / "negative_line_total.txt"
)
INVALID_RECEIPT_TOTAL_FORMAT_FIXTURE_PATH = (
    PROJECT_ROOT / "fixtures" / "inputs" / "invalid_receipt_total_format.txt"
)
...
INVALID_DUPLICATE_TOTAL_FIXTURE_PATH = (
    PROJECT_ROOT / "fixtures" / "inputs" / "invalid_duplicate_total.txt"
)
...
INVALID_DUPLICATE_MERCHANT_FIXTURE_PATH = (
    PROJECT_ROOT / "fixtures" / "inputs" / "invalid_duplicate_merchant.txt"
)
INVALID_DUPLICATE_DATE_FIXTURE_PATH = (
    PROJECT_ROOT / "fixtures" / "inputs" / "invalid_duplicate_date.txt"
)
...
def test_reject_unit_price_without_two_decimal_places():
    raw_text = INVALID_UNIT_PRICE_FIXTURE_PATH.read_text(encoding="utf-8")
...
def test_reject_non_convertible_unit_price():
    raw_text = NON_CONVERTIBLE_UNIT_PRICE_FIXTURE_PATH.read_text(encoding="utf-8")
...
def test_reject_receipt_total_with_comma_decimal_separator():
    raw_text = INVALID_RECEIPT_TOTAL_FORMAT_FIXTURE_PATH.read_text(encoding="utf-8")
    ...
    assert error.code == "invalid_receipt_total"
    assert isinstance(error.message, str)
    assert error.message.strip() != ""


def test_reject_non_convertible_line_total():
    raw_text = INVALID_LINE_TOTAL_FORMAT_FIXTURE_PATH.read_text(encoding="utf-8")
...
def test_reject_receipt_with_duplicate_total():
    raw_text = INVALID_DUPLICATE_TOTAL_FIXTURE_PATH.read_text(encoding="utf-8")
    ...
    assert error.code == "duplicate_total"
    assert isinstance(error.message, str)
    assert error.message.strip() != ""


def test_reject_receipt_without_merchant():
    raw_text = INVALID_MISSING_MERCHANT_FIXTURE_PATH.read_text(encoding="utf-8")
...
def test_reject_receipt_with_duplicate_merchant():
    raw_text = INVALID_DUPLICATE_MERCHANT_FIXTURE_PATH.read_text(encoding="utf-8")
    ...
    assert error.code == "duplicate_merchant"
    assert isinstance(error.message, str)
    assert error.message.strip() != ""


def test_reject_receipt_with_duplicate_date():
    raw_text = INVALID_DUPLICATE_DATE_FIXTURE_PATH.read_text(encoding="utf-8")
